...
;(function() {
	...
	Renderer.prototype.getstyle = function(tagName, parentTagName) {
		...
		if ( this.options.enableInlineStyles ) {
			var styleOpt = this.options.inlineStyles[tagName] || {};

			if ( parentTagName ) {
				merge( styleOpt, this.options.inlineStyles[parentTagName + ' ' + tagName]);
			}
			...
			if ( styleOpt ) {
				...
				for (key in styleOpt) {
					...
					if (Object.prototype.hasOwnProperty.call(styleOpt, key)) {
						out += key + ':' +styleOpt[key] + ';';
					}
				}
				out += '"';
			}
